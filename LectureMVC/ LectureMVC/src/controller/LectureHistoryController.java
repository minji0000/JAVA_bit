...
public class LectureHistoryController {
    ...
    // 학생, 과목별 점수만 반환하는 메소드
    public LectureHistoryDTO selectGrade(int lectureId, int studentId ) {
        LectureHistoryDTO lh = null;
        String query = "SELECT * FROM `lectureHistory` WHERE `lectureId` =? AND `studentId` = ?";
        ...
        try {
            ...
            if(rs.next()) {
                lh = new LectureHistoryDTO();
                lh.setGrade(rs.getString("grade"));
            }
